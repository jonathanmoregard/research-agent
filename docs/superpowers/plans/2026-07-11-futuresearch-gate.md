...
Deltas applied over the plan text above, from adversarial review of Tasks 1–4:

- **Size gate before parse (Task 4):** `forecast_results` checks `len(raw.encode(...)) > _MAX_CONTENT_BYTES` immediately after fetch, BEFORE `json.loads`/`_typed_skeleton`. Oversized content gets no parse work and ships no skeleton (`data=None, fields_withheld=0`) — no O(n) work on content the gate rejects, and no skeleton derived from oversized bytes.
- **Defensive parse/skeleton guard (Task 4):** the `json.loads` + `_typed_skeleton` block is wrapped in broad `try/except Exception` (json.loads can `RecursionError` on adversarially deep input); on exception `data, fields_withheld = None, 0` and the scan path continues. Only `type(exc).__name__` is logged. No exception can escape `forecast_results` un-genericized.
- **Skeleton hardening (Task 3 review):** one shared budget counter consumed by every KEPT entry — dict keys AND list items — with the slot reserved before recursing (hard cap under nesting); depth guard drops containers past `_SKELETON_MAX_DEPTH` wholesale (no `RecursionError`); the internal `_DROP` sentinel never escapes to callers (top-level drop normalizes to `(None, 1)`); non-finite floats (`Infinity`/`NaN`, accepted by `json.loads`) are dropped via `math.isfinite` so `json.dumps` downstream never emits non-standard JSON.
- **`_wrap_content` source charset guard (Task 1 review):** the `source` label is gated to `[A-Za-z0-9_-]{1,64}`, falling back to `"research-agent"` — it is interpolated into the wrap attribute and trusted head/tail system-reminders, so a free-charset source would be a wrap/system-reminder forgery vector.
- **`_write_quarantine_audit` reports_dir param (Task 4, sanctioned alternative):** instead of syncing `server.REPORTS_DIR` before each audit write, the writer takes an optional `reports_dir` parameter defaulting to its module global; the gate passes its own `REPORTS_DIR` explicitly.
